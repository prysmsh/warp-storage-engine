; Warp Storage Engine Windows Installer Script
; Generated by GoReleaser

!define APPNAME "Warp Storage Engine"
!define COMPANYNAME "Warp Storage Engine"
...
Section "install"
    ...
    FileWrite $0 "echo Installing Warp Storage Engine as Windows service...$\r$\n"
    FileWrite $0 "sc create "FoundationStorageEngine" binPath= `"$INSTDIR\warp-storage-engine.exe -config $INSTDIR\config\config.yaml`" DisplayName= `"Warp Storage Engine Service`" start= auto$\r$\n"
    ...
    FileWrite $0 "@echo off$\r$\n"
    FileWrite $0 "echo Stopping and removing Warp Storage Engine service...$\r$\n"
    ...
    CreateShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\warp-storage-engine.exe" "" ""
    ...
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\${APPNAME}" "DisplayIcon" "$\"$INSTDIR\warp-storage-engine.exe$\""
...
Section "uninstall"
    ...
    ; Remove files
    Delete "$INSTDIR\warp-storage-engine.exe"
